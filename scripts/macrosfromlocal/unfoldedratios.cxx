...
void unfoldedratios()
{
	...
	hresponse->GetYaxis()->SetTitle("p_{T}^{true} (GeV/#it{c})");
	hresponse->GetXaxis()->SetTitle("p_{T}^{meas} (GeV/#it{c})");
	hresponse->SetTitle("");
	hresponse->Draw("colz0");

	latex->DrawLatex(lleft, lbottom-3.5*lstep, Form("PYTHIA6 p+p #otimes STAR Au+Au %s", cent.Data()));
	latex->DrawLatex(lleft, lbottom-4.5*lstep, "#sqrt{s_{NN}} = 200 GeV");
	latex->DrawLatex(lleft, lbottom-5.5*lstep, Form("anti-k_{T}, R = %.1f", R));
	latex->DrawLatex(lleft, lbottom-6.5*lstep, Form("p_{T,min}^{lead} = %.0f GeV/#it{c}", pTlead));
	//latex->DrawLatex(lleft, lbottom-1*lstep, Form("prior: %s ", prior.Data()));
	...
	for (int i = 0; i < 8; i++)
	{
	 ...
	 hunfolded[i]->SetYTitle("dN/dp_{T} (GeV/#it{c})^{-1}");
	...
	//j+=5;	//increment by x each step
	 ...
	 hratio[i-1]->SetXTitle("p_{T,jet} (GeV/#it{c})");
  ...
  leg->SetFillColor(0);
	...
	latex->DrawLatex(lleft+5*lstep, lbottom+7*lstep, "THIS THESIS");
	latex->DrawLatex(lleft+5*lstep, lbottom+6*lstep, Form("STAR Au+Au %s", cent.Data()));
	latex->DrawLatex(lleft+6*lstep, lbottom+5*lstep, "#sqrt{s_{NN}} = 200 GeV");
	latex->DrawLatex(lleft, lbottom-0*lstep, Form("anti-k_{T}, R = %.1f", R));
	latex->DrawLatex(lleft, lbottom-lstep, Form("p_{T,min}^{lead} = %.0f GeV/#it{c}", pTlead));
  ...
  // pad2[cent]->SetGridx(); // vertical grid
	...
	hbm[7]->GetXaxis()->SetLabelSize(0.13);
	 ...
	 hbm[7]->SetXTitle("p_{T,jet} (GeV/#it{c})");
