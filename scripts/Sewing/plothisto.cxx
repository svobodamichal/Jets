...
void plothisto(string prod = "combined_response")
{
	...
	for(double &R : Rarr)
	{
        ...
        hEtaMc_Rc[k]->Add(hEtaMc_Rc[k]);
        hEtaMc_Rc[k]->Write();

        hPhiMc_Rc[k]->Add(hPhiMc_Rc[k]);
        hPhiMc_Rc[k]->Write();
        ...
        hEtaMc_Rcw[k]->Add(hEtaMc_Rcw[k]);
        hEtaMc_Rcw[k]->Write();

        hPhiMc_Rcw[k]->Add(hPhiMc_Rcw[k]);
        hPhiMc_Rcw[k]->Write();
		...
		TGaxis::SetMaxDigits(3);
			for(int cent = 1; cent < 8; cent++)
			{
                ...
                hMCNconst[k][cent]->Add(hMCNconst[k][cent]);
...
//				heffi[pTlead][k][cent]->Divide(hMcpT[pTlead][k][cent]);		
                ...
                hMcMatchedMCpT[pTlead][k][cent]=(TH1D*)list1->FindObject(Form("hMCmatchedpT_MCpTl%i_R0%.0f_centbin%i", pTlead, R*10,cent));
