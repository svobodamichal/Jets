...
void unfold(Int_t priorNo=2,Double_t pTlead =5.0,TH1D* hepsilon=NULL)
{
 ...
 //  TString prior_type[]={"", "mtsalis_1","mtsalis_2","mtsalis_3","mtsalis_4","mtsalis_5","mtsalis_6", "gauspol","expol","gammapol_1","gammapol_2","gammapol_3","gammapol_4","gammapol_5","gauspol_1","gauspol_2","gauspol_3","gauspol_4","gauspol_5"}; 
  ...
  bool use2Dmeasured=atoi(gSystem->Getenv("USE2DHISTO"));//use 2d histogram with measured spectrum
	...
	cout<<"pTleading value required: "<<pTlead<<" Prior: "<<priorNo<<endl;
	}	
...
//I/O FILES
  ...
  //str = Form("%s/histos_HT2_total.root", data_path.Data()); //before calib
  //str = Form("%s/histos_HT2_newcalib.root", data_path.Data()); //new BEMC calibration
  str = Form("%s/histos_HT2_tow_total.root", data_path.Data()); //latest (tower based version)
...
str =Form("%s/%s/unfolded_SignalSpectrum_R%.1lf_pTthresh%.1lf.root", out_dir.Data(),prior_type[priorNo].Data(), R, pTlead); 
...
str = Form("%s/pythia6_effi.root",epsilon_path.Data());
...
//hepsilon=(TH1D*) fepsilon->Get("hepsilon_transposed");
  ...
  //str = Form("%s/response_matrix_%s_R%.1lf_pTlead%.1lf_newPriors.root", rmatrix_path.Data(),rmatrix_type.Data(),R,pTlead); //TEST NEW PRIORS 
 	//str = Form("%s/response_matrix_%s_R%.1lf_pTlead%.1lf_%s_allPriors_test.root", rmatrix_path.Data(),rmatrix_type.Data(),R,pTlead,centrality.Data()); //test only
  //str = Form("%s/response_matrix_%s_R%.1lf_pTlead%.1lf_%s_allPriors.root", rmatrix_path.Data(),rmatrix_type.Data(),R,pTlead,centrality.Data()); //smooth matrix with specified centrality
...
//test only pT > 10
	...
	//cout << "hMCtrue_tmp Integral: " << hMCtrue->Integral() << endl;
	//cout << "Number of hMCtrue entries " << hMCtrue->GetEntries() << endl;
   delete hMCtrue_tmp;
//error calculation
