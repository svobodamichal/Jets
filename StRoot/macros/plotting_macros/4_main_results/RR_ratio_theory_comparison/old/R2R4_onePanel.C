...
void R2R4(C_systems system=cent/*this variable is only for compatibility with R2R4.C macro*/,float pTlead=5.0,int binning=1, TString evo="omicron",TString label="", TString ext="pdf"){
  ...
  //===========================================================================
	...
	for(int cent=0; cent<ncent; cent++)
	{
		...
		if(cent==1)gPad->SetLeftMargin(0.0);
		gPad->SetTicks(1);
		if(plotLogY)gPad->SetLogy();
  
		htmp1[cent]=new TH1D(Form("htmp%i",cent),"",100,0,xMax[cent]);
		htmp1[cent]->SetXTitle("p_{T, jet}  or  p_{T, jet}^{ch}  (GeV/c)");
		...
		legraa0->AddEntry(gjan[cent], Form("  STAR, p_{T,lead}^{min}= %.1lf GeV/c",pTlead),"lp");
		legraa0->AddEntry("","p+p #sqrt{s_{NN}}=200GeV","");
		if(showHerwig) 
		{
		...
		legraa1->SetTextSize(0.032);
		legraa1->SetFillStyle(0);
		legraa1->SetBorderSize(0);
		//legraa->AddEntry("",Form("    p_{T}^{lead}>%.1lf",pTlead),"");
		legraa1->SetHeader("Full jets, Au+Au #sqrt{s_{NN}}= 200 GeV");
			if(showVitevAuAu) legraa1->AddEntry(gvit1/*gAAR2R4up_dEdx*/, "  NLO pQCD","f");
			if(showVitevAuAu) legraa1->AddEntry(gvit2/*gAAR2R4up*/, "  SCET","f");
			if(showKrishna) legraa1->AddEntry(gKrishnaR2R4, "  Hybrid model", "f");
		cfig1->cd(2);
