void event_qa(TString cclass="cent")
{
   ...
   float zymin=-15;
	...
	for(int i=1; i<=hrefmultVSrunid1->GetYaxis()->GetNbins(); i++)
	{
		...
		float mrefmult=hslicerm1->GetMean();
	...
	for(int i=1; i<=hdayrefmult2D_1->GetXaxis()->GetNbins(); i++)
	{
		TH1D* hslice1=hdayrefmult2D_1->ProjectionY(Form("hslice1_%i",i),i,i);
		float mrefmult=hslice1->GetMean();
	...
	//Legend
	TLegend *legspectra = new TLegend(0.65, 0.75, 0.90, 0.90);
