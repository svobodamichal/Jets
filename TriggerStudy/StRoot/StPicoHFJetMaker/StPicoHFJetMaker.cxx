...
int StPicoHFJetMaker::InitJets() {
  ...
  // EXAMPLE //  TH1F* hist = static_cast<TH1F*>(mOutList->Last());
    ...
    for (int centbin = 1; centbin < 8; centbin++) {
        ...
        hname = Form("hPrimTrackHT2_centbin%i", centbin);
        mOutList->Add(new TH1D(hname, "primary track p_{T} for HT2; p_{T} [GeV/c]", npttrackbins, pttrackmin, pttrackmax));
        hname = Form("hPrimTrackHT3_centbin%i", centbin);
        mOutList->Add(new TH1D(hname, "primary track p_{T} for HT3; p_{T} [GeV/c]", npttrackbins, pttrackmin, pttrackmax));

        hname = Form("hPrimTowerMB_centbin%i", centbin);
        mOutList->Add(new TH1D(hname, "Tower energy E_{T} for MB; E_{T} [GeV/c^2]", npttrackbins, pttrackmin, pttrackmax));
        hname = Form("hPrimTowerHT1_centbin%i", centbin);
        mOutList->Add(new TH1D(hname, "Tower energy E_{T} for HT1; E_{T} [GeV/c^2]", npttrackbins, pttrackmin, pttrackmax));
        hname = Form("hPrimTowerHT2_centbin%i", centbin);
