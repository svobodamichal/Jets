plot_priors(Float_t pTthresh=5.0,TString suff="",TString input_suff="")
{ 
  gStyle->SetOptStat(0);
  gStyle->SetPalette(1);
  gStyle->SetOptDate(0);
  ...
  Int_t priors[]={5,3,8};
  const Int_t npriors=3;
  const Int_t npriors_py=3;
  ...
  str = Form("./root/histos_prior%s.root",input_suff.Data());
  TFile *fprior = new TFile(str.Data(), "OPEN");
  for(int i=0;i<npriors_py;i++){
	 str=Form("hprior_%s_R%.1lf_pTlead%.0lf",prior_type[2].Data(),R[i],pTthresh);
	 hprior_py[i] = (TH1D*)fprior->Get(str.Data());
	 	 str=Form("hprior_%s_R%.1lf_pTlead%.0lf",prior_type[2].Data(),R[i],0);

	 hprior_py0[i] = (TH1D*)fprior->Get(str.Data());
  ...
  for(int i=0;i<npriors;i++){
	  str=Form("hprior_%s_pTlead%.0lf",prior_type[priors[i]].Data(),pTthresh);
	  if(priors[i]==1 || priors[i]==2) str=Form("hprior_%s_R%.1lf_pTlead%.0lf",prior_type[priors[i]].Data(),R[1],pTthresh);
	  hprior[i] = (TH1D*)fprior->Get(str.Data());
